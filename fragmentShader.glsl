...
float qs_compare(vec4 qsa, vec4 qsb) {
	if (ds_compare(qsa.xy, qsb.xy) < 0.)
		return -1.;                            // if (dsa.x < dsb.x) return -1.;
	else if (ds_compare(qsa.xy, qsb.xy) == 0.) // else if (dsa.x == dsb.x)
	{
		if (ds_compare(qsa.zw, qsb.zw) < 0.)
			return -1.; // if (dsa.y < dsb.y) return -1.;
		else if (ds_compare(qsa.zw, qsb.zw) == 0.)
			return 0.; // else if (dsa.y == dsb.y) return 0.;
		else
			return 1.;
	} else
		return 1.;
	...
	vec4 q =
	    qs_add(
	        qs_mul(
	            qs_add(x, -_0_25),
	            qs_add(x, -_0_25)),
	        qs_mul(y, y)); // (x - 0.25f) * (x - 0.25f) + y * y
	        ...
	        qs_compare(
	            qs_add(
	                qs_mul(
	                    ...
	                    qs_add(x, _1)),
	                qs_mul(y, y)),
	            _1_16) < 0.0 // (x + 1) * (x + 1) + y * y < 1/16
	        ||
	        qs_compare(
	            qs_mul(
	                q,
	                qs_add(q, qs_add(x, -_0_25))),
	            qs_mul(_0_25, qs_mul(y, y))) < 0.0 // q * (q + x - 0.25) < 0.25 * y * y
	        )) {
		for (iteration = 0;
